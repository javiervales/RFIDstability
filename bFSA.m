function [probability,  elapsedtime] = bFSA(N,T,k,pc,ps)
% Montecarlo simulation for the computation of batch identification probability (bip)
% under FSA-policy with constant K value
% function [meanvalue, tol, confidencelevel] = bFSA(N,T,k,pc,ps)
% N: Batch size (Tags)
% T: Timeslot duration (seconds)
% Confidence interval for the bit is meanvalue +- tol (with confidence level of 99%)
